...
class DistributorForm
  ...
  def self.distributor_attributes
    Distributor.column_names + Distributor.reflections.keys
  end
  ...
  def self.model_name
    ActiveModel::Name.new(self, nil, 'Distributor')
  end

  # Delegates getter and setter methods for each of the attributes back to the original model
  distributor_attributes.each do |attr|
    delegate attr, "#{attr}=", to: :distributor
  end
  ...
  #
  # When the form and the distributor are both valid, the record is saved. Otherwise, the form displays all errors
  # found in validation process.
  #
  # @param [Hash] params - form's attributes and its values
  #
  # @return [Boolean] true when distributor is saved, false otherwise
  def submit(params)
    ...
    if completely_valid?
      ...
      promote_errors(distributor.errors)
      false
    end
  end
  ...
  # Performs the validation of both, the form itself and the distributor
  #
  ...
  #
  # @return [Boolean] true when form and distributor are valid, false otherwise
  def completely_valid?
    distributor_valid = distributor.valid?
    valid? && distributor_valid
  end

  # Takes all given validation errors found in distributor and adds them to errors of the form itself. These errors
  ...
  # it normally would.
  #
  # @param [ActiveModel::Errors] child_errors - validation errors found in distributor
