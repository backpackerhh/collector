...
class FormatForm
  ...
  def self.format_attributes
    Format.column_names + Format.reflections.keys
  end
  ...
  def self.model_name
    ActiveModel::Name.new(self, nil, 'Format')
  end

  # Delegates getter and setter methods for each of the attributes back to the original model
  format_attributes.each do |attr|
    delegate attr, "#{attr}=", to: :format
  end
  ...
  #
  # When the form and the format are both valid, the record is saved. Otherwise, the form displays all errors
  # found in validation process.
  #
  # @param [Hash] params - form's attributes and its values
  #
  # @return [Boolean] true when format is saved, false otherwise
  def submit(params)
  ...
  # Performs the validation of both, the form itself and the format
  #
  ...
  #
  # @return [Boolean] true when form and format are valid, false otherwise
  def completely_valid?
    format_valid = format.valid?
    valid? && format_valid
  end

  # Takes all given validation errors found in format and adds them to errors of the form itself. These errors
  ...
  # it normally would.
  #
  # @param [ActiveModel::Errors] child_errors - validation errors found in format
