...
class EditionForm
  ...
  def self.model_name
    ActiveModel::Name.new(self, nil, 'Edition')
  end

  # Delegates getter and setter methods for each of the attributes back to the original model
  edition_attributes.each do |attr|
    delegate attr, "#{attr}=", to: :edition
  end
  ...
  #
  # When the form and the edition are both valid, the record is saved. Otherwise, the form displays all errors
  # found in validation process.
  #
  # @param [Hash] params - form's attributes and its values
  #
  # @return [Boolean] true when edition is saved, false otherwise
  def submit(params)
  ...
  # Performs the validation of both, the form itself and the edition
  #
  ...
  #
  # @return [Boolean] true when form and edition are valid, false otherwise
  def completely_valid?
    edition_valid = edition.valid?
    valid? && edition_valid
  end

  # Takes all given validation errors found in edition and adds them to errors of the form itself. These errors
  ...
  # it normally would.
  #
  # @param [ActiveModel::Errors] child_errors - validation errors found in edition
